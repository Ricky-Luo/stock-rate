<template>
  <div>
    <v-card>
      <v-card-title style="font-size: 1.05em">编辑标签</v-card-title>
      <v-card-text>
        <v-container>
          <v-row>
            <v-col cols="3">
              <v-text-field
                ...
                label="标签名称"
                @click:append="saveRace"
              ></v-text-field>
            </v-col>
            <v-col cols="3">
              <v-text-field
                v-model="race.comment"
                label="备注"
              ></v-text-field>
            </v-col>
            <v-col cols="2">
          ...
          <!-- 股票关联操作部分 start -->
          <v-row v-show="race.id !== undefined" style="margin-top: -20px">
            <v-col cols="7">
            ...
            <v-col cols="5">
              ...
              >
                <v-card-title style="font-size: 1.1em"
                  >“{{ race.raceName }}”关联的股票</v-card-title
                >
                <v-card-text>

                  <div style="width: 100%; height: 318px; overflow: scroll">
                    <v-list dense color="#75A4BF">
                      <v-list-item-group
                        color="white"
...
export default {
  ...
  methods: {
    ...
    getRacesStock() {
      ...
      if (this.race.id) {
        dataInterface
          .getStocksByRaceid({ raceid: this.race.id })
          .then((res) => {
    ...
    selectAllPool() {
      let temp = [];
      this.stocksResult.forEach((e) => {
        temp.push(e.stockCode);
      });
      this.stockPooolChecked = temp;
    },
    // 反选候选池
    unSelectAllPool() {
      this.stockPooolChecked = [];
    },
    // 关联到赛道
    addToRace(item) {
      ...
      for (let i = 0; i < temp.length; i++) {
        if(temp[i].stockCode === item.stockCode) {
          add = false;
          break;
        }
      }
